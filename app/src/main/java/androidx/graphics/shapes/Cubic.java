...
public class Cubic {
    ...
    private Cubic(long j, long j2, long j3, long j4) {
        this(new float[]{PointKt.m103getXDnnuFBc(j), PointKt.m104getYDnnuFBc(j), PointKt.m103getXDnnuFBc(j2), PointKt.m104getYDnnuFBc(j2), PointKt.m103getXDnnuFBc(j3), PointKt.m104getYDnnuFBc(j3), PointKt.m103getXDnnuFBc(j4), PointKt.m104getYDnnuFBc(j4)});
    }

    /* JADX INFO: renamed from: pointOnCurve-OOQOV4g$graphics_shapes_release */
    public final long m91pointOnCurveOOQOV4g$graphics_shapes_release(float t) {
        float f = 1 - t;
        float f2 = f * f * f;
        float f3 = 3 * t;
        float f4 = f3 * f * f;
    ...
    /* JADX WARN: Removed duplicated region for block: B:153:0x01c9 A[PHI: r3 r7
      0x01c9: PHI (r3v15 float) = (r3v12 float), (r3v21 float) binds: [B:175:0x0218, B:152:0x01c7] A[DONT_GENERATE, DONT_INLINE]
      0x01c9: PHI (r7v14 float) = (r7v10 float), (r7v16 float) binds: [B:175:0x0218, B:152:0x01c7] A[DONT_GENERATE, DONT_INLINE]] */
    /* JADX WARN: Removed duplicated region for block: B:154:0x01cb  */
    /* JADX WARN: Removed duplicated region for block: B:177:0x021b A[PHI: r9 r15
      0x021b: PHI (r9v5 float) = (r9v2 float), (r9v3 float), (r9v3 float), (r9v2 float), (r9v2 float), (r9v2 float) binds: [B:155:0x01d2, B:167:0x0201, B:169:0x0205, B:141:0x01a6, B:144:0x01b0, B:146:0x01b4] A[DONT_GENERATE, DONT_INLINE]
      0x021b: PHI (r15v13 float) = (r15v9 float), (r15v10 float), (r15v10 float), (r15v9 float), (r15v9 float), (r15v9 float) binds: [B:155:0x01d2, B:167:0x0201, B:169:0x0205, B:141:0x01a6, B:144:0x01b0, B:146:0x01b4] A[DONT_GENERATE, DONT_INLINE]] */
    /*
    ...
    */
    ...
    public final Pair<Cubic, Cubic> split(float t) {
        float f = 1 - t;
        long jM91pointOnCurveOOQOV4g$graphics_shapes_release = m91pointOnCurveOOQOV4g$graphics_shapes_release(t);
        float f2 = f * f;
        float f3 = 2 * f * t;
        float f4 = t * t;
        return TuplesKt.to(CubicKt.Cubic(getAnchor0X(), getAnchor0Y(), (getAnchor0X() * f) + (getControl0X() * t), (getAnchor0Y() * f) + (getControl0Y() * t), (getAnchor0X() * f2) + (getControl0X() * f3) + (getControl1X() * f4), (getAnchor0Y() * f2) + (getControl0Y() * f3) + (getControl1Y() * f4), PointKt.m103getXDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release), PointKt.m104getYDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release)), CubicKt.Cubic(PointKt.m103getXDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release), PointKt.m104getYDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release), (getControl0X() * f2) + (getControl1X() * f3) + (getAnchor1X() * f4), (getControl0Y() * f2) + (getControl1Y() * f3) + (getAnchor1Y() * f4), (getControl1X() * f) + (getAnchor1X() * t), (getControl1Y() * f) + (getAnchor1Y() * t), getAnchor1X(), getAnchor1Y()));
    }
    ...
    public static final class Companion {
        ...
        public final Cubic circularArc(float centerX, float centerY, float x0, float y0, float x1, float y1) {
            ...
            long jM124rotate90DnnuFBc = Utils.m124rotate90DnnuFBc(jDirectionVector);
            long jM124rotate90DnnuFBc2 = Utils.m124rotate90DnnuFBc(jDirectionVector2);
            boolean z = PointKt.m98dotProduct5P9i7ZU(jM124rotate90DnnuFBc, f3, f4) >= 0.0f;
            float fM99dotProductybeJwSQ = PointKt.m99dotProductybeJwSQ(jDirectionVector, jDirectionVector2);
            if (fM99dotProductybeJwSQ > 0.999f) {
                return straightLine(x0, y0, x1, y1);
            }
            float fDistance = ((((Utils.distance(f, f2) * 4.0f) / 3.0f) * (((float) Math.sqrt(2 * r9)) - ((float) Math.sqrt(r5 - (fM99dotProductybeJwSQ * fM99dotProductybeJwSQ))))) / (1 - fM99dotProductybeJwSQ)) * (z ? 1.0f : -1.0f);
            return CubicKt.Cubic(x0, y0, x0 + (PointKt.m103getXDnnuFBc(jM124rotate90DnnuFBc) * fDistance), y0 + (PointKt.m104getYDnnuFBc(jM124rotate90DnnuFBc) * fDistance), x1 - (PointKt.m103getXDnnuFBc(jM124rotate90DnnuFBc2) * fDistance), y1 - (PointKt.m104getYDnnuFBc(jM124rotate90DnnuFBc2) * fDistance), x1, y1);
        }
    }
}
