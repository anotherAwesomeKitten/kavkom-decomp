...
public final class RoundedPolygon {
    ...
    public final RoundedPolygon transformed(PointTransformer f) {
        ...
        long jM110transformedso9K2fw = PointKt.m110transformedso9K2fw(FloatFloatPair.m10constructorimpl(this.centerX, this.centerY), f);
        List listCreateListBuilder = CollectionsKt.createListBuilder();
        int size = this.features.size();
        for (int i = 0; i < size; i++) {
            listCreateListBuilder.add(this.features.get(i).transformed$graphics_shapes_release(f));
        }
        return new RoundedPolygon(CollectionsKt.build(listCreateListBuilder), PointKt.m103getXDnnuFBc(jM110transformedso9K2fw), PointKt.m104getYDnnuFBc(jM110transformedso9K2fw));
    }
    ...
    public final RoundedPolygon normalized() {
        ...
        return transformed(new PointTransformer() { // from class: androidx.graphics.shapes.RoundedPolygon.normalized.1
            ...
            public final long mo112transformXgqJiTY(float f6, float f7) {
                float f8 = f6 + f4;
                float f9 = fMax;
    ...
    public final float[] calculateMaxBounds(float[] bounds) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            long jM91pointOnCurveOOQOV4g$graphics_shapes_release = cubic.m91pointOnCurveOOQOV4g$graphics_shapes_release(0.5f);
            fMax = Math.max(fMax, Math.max(fDistanceSquared, Utils.distanceSquared(PointKt.m103getXDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release) - this.centerX, PointKt.m104getYDnnuFBc(jM91pointOnCurveOOQOV4g$graphics_shapes_release) - this.centerY)));
        }
