...
public class JobSupport implements Job, ChildJob, ParentJob {
    ...
    private final Object finalizeFinishingState(Finishing finishing, Object proposedUpdate) throws Throwable {
        ...
        boolean zM = AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_state$FU, this, finishing, JobSupportKt.boxIncomplete(proposedUpdate));
        if (DebugKt.getASSERTIONS_ENABLED() && !zM) {
            throw new AssertionError();
        }
        completeStateFinalization(finishing, proposedUpdate);
        return proposedUpdate;
    }

    private final Throwable getFinalRootCause(Finishing finishing, List<? extends Throwable> exceptions) {
        Object next;
        Object obj = null;
    ...
    private final boolean tryFinalizeSimpleState(Incomplete incomplete, Object update) throws Throwable {
        ...
        if (!AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_state$FU, this, incomplete, JobSupportKt.boxIncomplete(update))) {
            return false;
        }
        onCancelling(null);
        onCompletionInternal(update);
        completeStateFinalization(incomplete, update);
        return true;
    }

    private final void completeStateFinalization(Incomplete incomplete, Object update) throws Throwable {
        ChildHandle parentHandle$kotlinx_coroutines_core = getParentHandle$kotlinx_coroutines_core();
        if (parentHandle$kotlinx_coroutines_core != null) {
    ...
    */
    ...
    private final void promoteEmptyToNodeList(Empty empty) {
        ...
        AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_state$FU, this, empty, empty.getIsActive() ? nodeList : new InactiveNodeList(nodeList));
    }
    ...
    private final boolean tryMakeCancelling(Incomplete incomplete, Throwable rootCause) throws Throwable {
        if (DebugKt.getASSERTIONS_ENABLED() && (incomplete instanceof Finishing)) {
            throw new AssertionError();
        }
        if (DebugKt.getASSERTIONS_ENABLED() && !incomplete.getIsActive()) {
            throw new AssertionError();
        }
        ...
        if (!AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_state$FU, this, incomplete, new Finishing(orPromoteCancellingList, false, rootCause))) {
            return false;
        }
    ...
    private final Object tryMakeCompleting(Object obj, Object proposedUpdate) {
        if (!(obj instanceof Incomplete)) {
            return JobSupportKt.COMPLETING_ALREADY;
        }
        if ((!(obj instanceof Empty) && !(obj instanceof JobNode)) || (obj instanceof ChildHandleNode) || (proposedUpdate instanceof CompletedExceptionally)) {
            return tryMakeCompletingSlowPath((Incomplete) obj, proposedUpdate);
        }
        return tryFinalizeSimpleState((Incomplete) obj, proposedUpdate) ? proposedUpdate : JobSupportKt.COMPLETING_RETRY;
    }

    /* JADX WARN: Multi-variable type inference failed */
    private final Object tryMakeCompletingSlowPath(Incomplete incomplete, Object proposedUpdate) throws Throwable {
        NodeList orPromoteCancellingList = getOrPromoteCancellingList(incomplete);
        if (orPromoteCancellingList == null) {
    ...
    private final boolean tryWaitForChild(Finishing finishing, ChildHandleNode child, Object proposedUpdate) {
        while (Job.DefaultImpls.invokeOnCompletion$default(child.childJob, false, false, new ChildCompletion(this, finishing, child, proposedUpdate), 1, null) == NonDisposableHandle.INSTANCE) {
    ...
    public final void continueCompleting(Finishing finishing, ChildHandleNode lastChild, Object proposedUpdate) {
        if (DebugKt.getASSERTIONS_ENABLED() && getState$kotlinx_coroutines_core() != finishing) {
            throw new AssertionError();
        }
        ...
        if (childHandleNodeNextChild == null || !tryWaitForChild(finishing, childHandleNodeNextChild, proposedUpdate)) {
            afterCompletion(finalizeFinishingState(finishing, proposedUpdate));
        }
    }
    ...
    private final String stateString(Object obj) {
        if (!(obj instanceof Finishing)) {
            return obj instanceof Incomplete ? ((Incomplete) obj).getIsActive() ? "Active" : "New" : obj instanceof CompletedExceptionally ? "Cancelled" : "Completed";
        }
