...
public abstract class Random {
    ...
    public int nextInt(int i, int until) {
        int iNextInt;
        int i2;
        int iNextBits;
        RandomKt.checkRangeBounds(i, until);
        int i3 = until - i;
        ...
        while (true) {
            ...
            if (i <= iNextInt2 && iNextInt2 < until) {
                return iNextInt2;
            }
        }
    }
    ...
    public long nextLong(long j, long until) {
        long jNextLong;
        long j2;
        long jNextBits;
        int iNextInt;
        RandomKt.checkRangeBounds(j, until);
        long j3 = until - j;
        if (j3 > 0) {
            if (((-j3) & j3) == j3) {
                int i = (int) j3;
        ...
        while (true) {
            ...
            if (j <= jNextLong2 && jNextLong2 < until) {
                return jNextLong2;
            }
        }
    }
    ...
    public double nextDouble(double d, double until) {
        double dNextDouble;
        RandomKt.checkRangeBounds(d, until);
        double d2 = until - d;
        if (Double.isInfinite(d2) && Math.abs(d) <= Double.MAX_VALUE && Math.abs(until) <= Double.MAX_VALUE) {
            double d3 = 2;
            double dNextDouble2 = nextDouble() * ((until / d3) - (d / d3));
            dNextDouble = d + dNextDouble2 + dNextDouble2;
        } else {
            dNextDouble = d + (nextDouble() * d2);
        }
    ...
    public static final class Companion extends Random implements Serializable {
        ...
        @Override // kotlin.random.Random
        public int nextInt(int i, int until) {
            return Random.defaultRandom.nextInt(i, until);
        }
        ...
        @Override // kotlin.random.Random
        public long nextLong(long j, long until) {
            return Random.defaultRandom.nextLong(j, until);
        }
        ...
        @Override // kotlin.random.Random
        public double nextDouble(double d, double until) {
            return Random.defaultRandom.nextDouble(d, until);
        }
