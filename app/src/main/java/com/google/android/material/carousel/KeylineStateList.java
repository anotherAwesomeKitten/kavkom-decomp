    ...
    private static KeylineState shiftKeylineStateForPaddingUncontained(KeylineState keylineState, float f, int i, boolean z) {
        ...
        while (i2 < arrayList.size()) {
            ...
            if (keyline.isAnchor && i2 == size) {
                ...
                if (z) {
                    fMin = Math.max(0.0f, ((keyline.maskedItemSize / 2.0f) + f6) - i);
                } else {
                    fMin = Math.min(0.0f, f6 - (keyline.maskedItemSize / 2.0f));
                }
                builder.addKeyline(f6, f7, f8, z2, z3, Math.abs(fMin), f4, f5);
            }
            i2++;
        }
