    ...
    */
    ...
    protected void calculateRootLayout(ReactShadowNode reactShadowNode) {
        ...
        try {
            ...
            if (View.MeasureSpec.getMode(iIntValue) == 0) {
                size = YogaConstants.UNDEFINED;
            } else {
                size = View.MeasureSpec.getSize(iIntValue);
            }
            if (View.MeasureSpec.getMode(iIntValue2) == 0) {
                size2 = YogaConstants.UNDEFINED;
            } else {
                size2 = View.MeasureSpec.getSize(iIntValue2);
            }
            reactShadowNode.calculateLayout(size, size2);
        } finally {
            Systrace.endSection(0L);
