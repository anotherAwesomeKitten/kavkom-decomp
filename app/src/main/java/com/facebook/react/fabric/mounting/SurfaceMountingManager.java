...
public class SurfaceMountingManager {
    ...
    private final SparseArrayCompat<Map<String, Object>> mTagToSynchronousMountProps = new SparseArrayCompat<>();
    ...
    private static Map<String, Object> getHashMapFromPropsReadableMap(ReadableMap readableMap) {
        ...
        if (readableMap.hasKey("opacity") && readableMap.getType("opacity") == ReadableType.Number) {
            map.put("opacity", Double.valueOf(readableMap.getDouble("opacity")));
        }
        return map;
    }

    public void storeSynchronousMountPropsOverride(int i, ReadableMap readableMap) {
        if (ReactNativeFeatureFlags.overrideBySynchronousMountPropsAtMountingAndroid()) {
            ...
            if (this.mTagToSynchronousMountProps.containsKey(i)) {
                Map<String, Object> map = (Map) Assertions.assertNotNull(this.mTagToSynchronousMountProps.get(i));
                map.putAll(hashMapFromPropsReadableMap);
                this.mTagToSynchronousMountProps.put(i, map);
                return;
            }
            this.mTagToSynchronousMountProps.put(i, hashMapFromPropsReadableMap);
        }
    }

    public void updatePropsSynchronously(int i, ReadableMap readableMap) {
        updateProps(i, readableMap, true);
    }

    public void updateProps(int i, ReadableMap readableMap) {
        updateProps(i, readableMap, false);
    }

    private void updateProps(int i, ReadableMap readableMap, Boolean bool) {
        if (isStopped()) {
            return;
        }
        ViewState viewState = getViewState(i);
        if (ReactNativeFeatureFlags.overrideBySynchronousMountPropsAtMountingAndroid() && !bool.booleanValue() && this.mTagToSynchronousMountProps.containsKey(i)) {
            WritableNativeMap writableNativeMap = new WritableNativeMap();
            writableNativeMap.merge(readableMap);
            overridePropsReadableMap((Map) Assertions.assertNotNull(this.mTagToSynchronousMountProps.get(i)), writableNativeMap);
    ...
    public void experimental_prefetchResources(int i, String str, MapBuffer mapBuffer) {
        if (isStopped()) {
            return;
        }
        this.mViewManagerRegistry.get(str).experimental_prefetchResources(i, (ThemedReactContext) Assertions.assertNotNull(this.mThemedReactContext), mapBuffer);
    }
    ...
    */
    ...
    public void deleteView(int i) {
        ...
        if (ReactNativeFeatureFlags.overrideBySynchronousMountPropsAtMountingAndroid() && this.mTagToSynchronousMountProps.containsKey(i)) {
            this.mTagToSynchronousMountProps.remove(i);
        }
        ViewState nullableViewState = getNullableViewState(i);
        if (nullableViewState == null) {
