...
public final class Hub implements IHub, MetricsApi.IMetricsInterface {
            ...
            if (z) {
                executorService.submit(new Runnable() { // from class: io.sentry.Hub$$ExternalSyntheticLambda1
                    ...
                    public final void run() {
                        this.f$0.m826lambda$close$1$iosentryHub(executorService);
                    }
                });
            } else {
    ...
    /* synthetic */ void m826lambda$close$1$iosentryHub(ISentryExecutorService iSentryExecutorService) {
    ...
    public void pushScope() {
        ...
        Stack.StackItem stackItemPeek = this.stack.peek();
        this.stack.push(new Stack.StackItem(this.options, stackItemPeek.getClient(), stackItemPeek.getScope().m833clone()));
    }
    ...
    private IScope buildLocalScope(IScope iScope, ScopeCallback scopeCallback) {
        if (scopeCallback != null) {
            try {
                IScope iScopeM833clone = iScope.m833clone();
                scopeCallback.run(iScopeM833clone);
                return iScopeM833clone;
            } catch (Throwable th) {
