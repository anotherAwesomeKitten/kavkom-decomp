...
public final class Scope implements IScope {
    ...
    public Session withSession(IWithSession iWithSession) {
        Session sessionM843clone;
        synchronized (this.sessionLock) {
            iWithSession.accept(this.session);
            sessionM843clone = this.session != null ? this.session.m843clone() : null;
        }
        return sessionM843clone;
    }
    ...
    public SessionPair startSession() {
        ...
        synchronized (this.sessionLock) {
            ...
            if (this.options.getRelease() != null) {
                ...
                sessionPair = new SessionPair(this.session.m843clone(), session != null ? session.m843clone() : null);
            } else {
    ...
    public Session endSession() {
        ...
        synchronized (this.sessionLock) {
            ...
            if (this.session != null) {
                this.session.end();
                Session sessionM843clone = this.session.m843clone();
                this.session = null;
                session = sessionM843clone;
            }
        }
